...
/*------------------------------------------------------------------
[numbat - SHORTCODES]
Project:    numbat – Multi-Purpose WordPress Template
Author:     ModelTheme
[Table of contents]
1. Recent Tweets
2. Contact Form
...
/*--- 1. Recent Tweets ---*/
/*---------------------------------------------*/
function numbat_twitter_time($a) {
...
function numbat_setup_shortcode_tweetslider( $params, $content ) {
    ...
    $tw_username            = $numbat_redux['numbat_social_tw'];
    $consumer_key           = $numbat_redux['numbat_tw_consumer_key'];
    $consumer_secret        = $numbat_redux['numbat_tw_consumer_secret'];
    $access_token           = $numbat_redux['numbat_tw_access_token'];
    $access_token_secret    = $numbat_redux['numbat_tw_access_token_secret'];
    $no = $no+1;
            ...
            foreach($tweets as $tweet) {
                ...
                $myContent .= '<div class="single-tweet">';
                    ...
                    $myContent .= '<div class="vc_col-md-10">';
                        ...
                        $myContent .= '<span class="tweet-date">'.numbat_twitter_time($created_at).'</span>';
                    $myContent .= '</div>';
                $myContent .= '</div>';
            }
            $myContent .= '</div>';
...
add_shortcode('tweets', 'numbat_setup_shortcode_tweetslider');
/*---------------------------------------------*/
/*--- 2. Contact Form ---*/
/*---------------------------------------------*/
function numbat_contact_form_shortcode($params, $content) {
    ...
    if (isset($_POST['contact_me'])) {
        $to = $numbat_redux['numbat_contact_email'];
    ...
    $contact_form .= '<form method="POST" class="animateIn" id="contact_form" novalidate="novalidate" data-animate="'.$animation.'">';
        ...
        $contact_form .= '<div class="vc_col-md-4">';
            $contact_form .= '<input type="text" placeholder="'.__('Your Name','numbat').'" class="form-control" name="user_name">';
        $contact_form .= '</div>';
        #Email
        $contact_form .= '<div class="vc_col-md-4">';
            $contact_form .= '<input type="text" placeholder="'.__('Your Email','numbat').'" class="form-control" name="user_email">';
        $contact_form .= '</div>';
        #Subject
        $contact_form .= '<div class="vc_col-md-4">';
            $contact_form .= '<input type="text" placeholder="'.__('Subject','numbat').'" class="form-control" name="user_subject">';
        $contact_form .= '</div>';
        $contact_form .= '<div class="mt-half-spacer"></div>';
        #Message
        $contact_form .= '<div class="vc_col-md-12">';
            $contact_form .= '<textarea name="user_message" rows="10" placeholder="'.__('Your Message','numbat').'" class="form-control"></textarea>';
        $contact_form .= '</div>';
        $contact_form .= '<div class="mt-half-spacer"></div>';
        #Submit button
        $contact_form .= '<div class="vc_col-md-12">';
            $contact_form .= '<input type="submit" class="solid-button button form-control" value="Send Now" name="contact_me">';
            $contact_form .= '<p class="success_message">'.__('Thank you! We\'ll get back to you as soon as possible.','numbat').'</p>';
        $contact_form .= '</div>';
    $contact_form .= '</form>';
    return $contact_form;
}
add_shortcode('contact_form', 'numbat_contact_form_shortcode');

/*---------------------------------------------*/
/*--- 4. Recent Posts ---*/
/*---------------------------------------------*/
function numbat_posts_calendar_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'animation'     => '',
            'title'     => '',
...
add_shortcode('posts_calendar', 'numbat_posts_calendar_shortcode');
/*---------------------------------------------*/
/*--- 5. Featured Post with thumbnail ---*/
/*---------------------------------------------*/
function numbat_featured_post_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'animation'      => '',
            'icon'      => '',
    ...
    $user_info = get_userdata($post_author_id);
    $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $postid ),'numbat_featured_post_pic500x230' );
    $featured_post .= '<div class="latest-videos animateIn" data-animate="'.$animation.'">';
        ...
        $featured_post .= '<div class="video-title">';
            ...
            $featured_post .= '</div>';
        $featured_post .= '<div class="video-excerpt">'.numbat_excerpt_limit($content,20).'</div>';
    $featured_post .= '</div>';
    return $featured_post;
}
add_shortcode('featured_post', 'numbat_featured_post_shortcode');
/*---------------------------------------------*/
/*--- 6. Testimonials ---*/
/*---------------------------------------------*/
function numbat_testimonials_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'animation'                 =>'',
            'number'                    =>'',
            ...
            foreach ($testimonials as $testimonial) {
                ...
                $myContent .= '<div class="item vc_col-md-12">';
                    $myContent .= '<div class="text-left">';
                        $myContent .= '<div class="testimonial-img-holder">';
...
add_shortcode('testimonials', 'numbat_testimonials_shortcode');
/*---------------------------------------------*/
/*--- 7. Subscribe form ---*/
/*---------------------------------------------*/
function numbat_subscribe_form($params, $content) {
    extract( shortcode_atts( 
        array(
            'animation'                  => '',
            'placeholder'                => '',
...
if (isset($_POST['mailchimp_email'])) {
    ...
    $apiKey         = $numbat_redux['numbat_mailchimp_apikey'];
    $listId         = $numbat_redux['numbat_mailchimp_listid'];
    $double_optin   = false;
    $send_welcome   = false;
    $email_type     = 'html';
    $email          = $_POST['mailchimp_email'];
    //replace us2 with your actual datacenter
    $submit_url = "http://" . $numbat_redux['numbat_mailchimp_data_center'] . ".api.mailchimp.com/1.3/?method=listSubscribe";
    $data = array(
        'email_address' => $email,
        'apikey'        => $apiKey,
        'id'            => $listId,
    ...
    if ($data->error){
        ...
        echo esc_attr__('Got it, you\'ve been added to our email list.', 'numbat');
    }
}
...
add_shortcode('subscribe_form', 'numbat_subscribe_form');
...
/*--- 7. Subscribe form ---*/
/*---------------------------------------------*/
function numbat_subscribe_form2($params, $content) {
    extract( shortcode_atts( 
        array(
            'animation' => '',
            'placeholder' => '',
...
if (isset($_POST['mailchimp_email'])) {
    ...
    $service = '';
    $service .= '<style type="text/css" scoped>
        .mc_embed_signup .newspaper-info {
            background: '.$email_box_background.'!important;
        }
...
add_shortcode('subscribe_form2', 'numbat_subscribe_form2');
/*---------------------------------------------*/
/*--- 8. Services style 1 ---*/
/*---------------------------------------------*/
function numbat_service_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'icon'          => '', 
            'title'         => '', 
...
add_shortcode('service', 'numbat_service_shortcode');
/*---------------------------------------------*/
/*--- 9. Services style 2 ---*/
/*---------------------------------------------*/
function numbat_service_style2_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'icon'          => '', 
            'title'         => '', 
...
add_shortcode('service_style2', 'numbat_service_style2_shortcode');

/*---------------------------------------------*/
/*--- 11. Recent testimonials ---*/
/*---------------------------------------------*/
function numbat_testimonials2_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'number'=>'',
            'animation'=>''
...
add_shortcode('testimonials-style2', 'numbat_testimonials2_shortcode');
/*---------------------------------------------*/
/*--- 12. Skill ---*/
/*---------------------------------------------*/
function numbat_skills_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'animation'          => '', 
            'icon'          => '', 
...
add_shortcode('skill', 'numbat_skills_shortcode');

/*---------------------------------------------*/
/*--- 14. Pricing tables ---*/
/*---------------------------------------------*/
function numbat_pricing_table_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'package_currency'  => '',
            'package_price'     => '',
...
add_shortcode('pricing-table', 'numbat_pricing_table_shortcode');
/*---------------------------------------------*/
/*--- 15. Jumbotron ---*/
/*---------------------------------------------*/
function numbat_jumbotron_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'heading'       => '', 
            'sub_heading'   => '', 
...
add_shortcode('jumbotron', 'numbat_jumbotron_shortcode');
/*---------------------------------------------*/
/*--- 16. Alert ---*/
/*---------------------------------------------*/
function numbat_alert_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'alert_style'           => '', 
...
add_shortcode('alert', 'numbat_alert_shortcode');
/*---------------------------------------------*/
/*--- 17. Progress bars ---*/
/*---------------------------------------------*/
function numbat_progress_bar_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
...
add_shortcode('progress_bar', 'numbat_progress_bar_shortcode');
/*---------------------------------------------*/
/*--- 18. Custom content ---*/
/*---------------------------------------------*/
function numbat_panel_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
...
<?php }
add_shortcode('panel', 'numbat_panel_shortcode');
/*---------------------------------------------*/
/*--- 20. Heading With Border ---*/
/*---------------------------------------------*/
function numbat_heading_with_border( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'align'       => 'left',
            'animation'   => ''
...
add_shortcode('heading-border', 'numbat_heading_with_border');
/*---------------------------------------------*/
/*--- 21. Testimonials ---*/
/*---------------------------------------------*/
function numbat_clients_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'animation'=>'',
            'number'=>''
...
add_shortcode('clients', 'numbat_clients_shortcode');
/*---------------------------------------------*/
/*--- 22. List group ---*/
/*---------------------------------------------*/
function numbat_list_group_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'heading'       => '',
            'description'   => '',
...
add_shortcode('list_group', 'numbat_list_group_shortcode');

function numbat_btn_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'btn_text'      => '',
            'btn_url'       => '',
...
add_shortcode('numbat_btn', 'numbat_btn_shortcode');
/*---------------------------------------------*/
/*--- 23. Thumbnails custom content ---*/
/*---------------------------------------------*/
function numbat_thumbnails_custom_content_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'image'         => '',
            'heading'       => '',
...
add_shortcode('thumbnails_custom_content', 'numbat_thumbnails_custom_content_shortcode');
/*---------------------------------------------*/
/*--- 24. Section heading with title and subtitle ---*/
/*---------------------------------------------*/
function numbat_heading_title_subtitle_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'title'         => '',
            'subtitle'      => ''
...
add_shortcode('heading_title_subtitle', 'numbat_heading_title_subtitle_shortcode');
/*---------------------------------------------*/
/*--- 25. Heading with bottom border ---*/
/*---------------------------------------------*/
function numbat_heanding_bottom_border_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'heading'    => '',
            'text_align' => ''
...
add_shortcode('heading_border_bottom', 'numbat_heanding_bottom_border_shortcode');
/*---------------------------------------------*/
/*--- 26. Portfolio square ---*/
/*---------------------------------------------*/
function numbat_portfolio_sqare_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'number'       => ''
           ), $params ) 
    ...
    foreach ( $posts->posts as $portfolio ) {
        ...
        $thumbnail_src      = wp_get_attachment_image_src( get_post_thumbnail_id( $portfolio->ID ), 'numbat_portfolio_pic700x450' );
        $content .= '<article id="post-'.$portfolio->ID.'" class="vc_col-md-4 single-portfolio-item numbat-item relative portfolio">';
        ...
        if($thumbnail_src) { 
            ...
            $content .= '<div class="item-description absolute">';
                $content .= '<div class="holder-top">';
                    $content .= '<a class="numbat-trigger" href="#"><i class="fa fa-expand"></i></a>';
            ...
            $content .= '<div class="numbat-quick-view portfolio-shortcode high-padding post-'.$portfolio->ID.'">';
                $content .= '<div class="numbat-slider-wrapper">';
                    $content .= '<ul class="numbat-slider">';
                        if($thumbnail_src) { 
                        ...
                        if( class_exists('Dynamic_Featured_Image') ) {
                            ...
                            foreach ($featured_images as $row=>$innerArray) {
                                ...
                                $mediumSizedImage = $dynamic_featured_image->get_image_url($id,'numbat_portfolio_pic700x450'); 
                    ...
                    $content .= '</ul>';
                    $content .= '<ul class="numbat-slider-navigation">';
                        $content .= '<li><a class="numbat-next" href="#0"><i class="fa fa-angle-left"></i></a></li>';
                        $content .= '<li><a class="numbat-prev" href="#0"><i class="fa fa-angle-right"></i></a></li>';
                    $content .= '</ul>';
                $content .= '</div>';

                $content .= '<div class="numbat-item-info col-md-5">';
                    ...
                    $content .= '<div class="portfolio-details">';
                        $content .= '<div class="vc_row">';
                            $content .= '<div class="vc_col-md-4 portfolio_label">'.esc_attr__('Customer:', 'numbat').'</div>';
                            $content .= '<div class="vc_col-md-8 portfolio_label_value">'.get_the_author().'</div>';
                        $content .= '</div>';
                        $content .= '<div class="vc_row">';
                            $content .= '<div class="vc_col-md-4 portfolio_label">'.esc_attr__('Live demo:', 'numbat').'</div>';
                            $content .= '<div class="vc_col-md-8 portfolio_label_value">'.$project_url.'</div>';
                        $content .= '</div>';
                        $content .= '<div class="vc_row">';
                            $content .= '<div class="vc_col-md-4 portfolio_label">'.esc_attr__('Skills:', 'numbat').'</div>';
                            $content .= '<div class="vc_col-md-8 portfolio_label_value">'.$project_skills.'</div>';
                        $content .= '</div>';
                        $content .= '<div class="vc_row">';
                            $content .= '<div class="vc_col-md-4 portfolio_label">'.esc_attr__('Date post:', 'numbat').'</div>';
                ...
                $content .= '</div>';
                $content .= '<a href="#0" class="numbat-close"><i class="fa fa-times"></i></a>';
            $content .= '</div>';
        $content .= '</article>';
    }
...
add_shortcode('portfolio-square', 'numbat_portfolio_sqare_shortcode');
/*---------------------------------------------*/
/*--- 27. Call to action ---*/
/*---------------------------------------------*/
function numbat_call_to_action_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'heading'       => '',
            'heading_type'  => '',
    ...
    $shortcode_content = '<div class="numbat_call-to-action">';
...
add_shortcode('numbat-call-to-action', 'numbat_call_to_action_shortcode');


/*---------------------------------------------*/
/*--- 27. Call to action ---*/
/*---------------------------------------------*/
function numbat_shop_feature_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'heading'       => '',
            'subheading'    => '',
...
add_shortcode('shop-feature', 'numbat_shop_feature_shortcode');
...
function numbat_shop_categories_with_thumbnails_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'number'                               => '',
            'category'                             => '',
...
add_shortcode('shop-categories-with-thumbnails', 'numbat_shop_categories_with_thumbnails_shortcode');
...
/*--- Masonry Banners ---*/
/*---------------------------------------------*/
function numbat_shop_masonry_banners_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            ...
            $shortcode_content .= '<div class="masonry_banner default-skin">';
                $shortcode_content .= '<a href="'.$banner_1_url.'" class="relative">';
                    ...
                    $shortcode_content .= '<div class="masonry_holder">';
                        ...
                        $shortcode_content .= '<span class="read-more">'.esc_attr__('VIEW MORE', 'numbat').'</span>';
                    $shortcode_content .= '</div>';
                $shortcode_content .= '</a>';
            $shortcode_content .= '</div>';
            #IMG #2
...
add_shortcode('shop-masonry-banners', 'numbat_shop_masonry_banners_shortcode');
...
/*--- Masonry Banners ---*/
/*---------------------------------------------*/
function numbat_shop_sale_banner_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'banner_img'            => '',
            'banner_button_text'    => '',
...
add_shortcode('sale-banner', 'numbat_shop_sale_banner_shortcode');
...
/*--- 28. BLOG POSTS ---*/
/*---------------------------------------------*/
function numbat_show_blog_post_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array(
            'number'            => '',
            'category'          => '',
    ...
    $shortcode_content = '';
    $shortcode_content .= '<div class="numbat_shortcode_blog vc_row sticky-posts">';
    foreach ($posts as $post) { 
        $excerpt = get_post_field('post_content', $post->ID);
        $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'numbat_portfolio_pic700x450' );
        ...
        $shortcode_content .= '<div class="'.$columns.' post">';
            $shortcode_content .= '<a href="'.$url.'" class="relative">';
                ...
                $shortcode_content .= '</div>';
            $shortcode_content .= '</a>';
            $shortcode_content .= '<h3 class="post-name"><a href="'.$url.'" style="color: '.$text_color.'!important;">'.$post->post_title.'</a></h3>';
            $shortcode_content .= '<div class="post-author">by '.get_the_author_meta( 'display_name', $author_id ) . ' / ' .get_the_date( "j M", $post->ID ).'</div>';
            $shortcode_content .= '<div class="post-excerpt">'.wp_trim_words($excerpt,20).'</div>';
        $shortcode_content .= '</div>';
    } 
    $shortcode_content .= '</div>';
    return $shortcode_content;
}
add_shortcode('numbat-blog-posts', 'numbat_show_blog_post_shortcode');
/*---------------------------------------------*/
/*--- 29. Social Media ---*/
/*---------------------------------------------*/
function numbat_social_icons_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'facebook'      => '',
            'twitter'       => '',
...
add_shortcode('social_icons', 'numbat_social_icons_shortcode');
                                ...
                                <div class="members_img_holder">
                                    ...
                                    <div class="member01-content">
                                        <div class="member01-content-inside">
                                            ...
                                            <div class="content-div"><p class="member01_content-desc">'. $content . '</p></div>
